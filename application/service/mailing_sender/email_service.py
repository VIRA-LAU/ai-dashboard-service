...
def send_mail(userId: str, link: str) -> bool:  # to be replaced by user Id that we get from Pi
    try:
        email_receiver = ""
        for doc in docs:
            if (doc.id == userId):
                print(doc.to_dict())
                email_receiver = doc.to_dict()['email']
        ...
        print(e.__str__(), "An error occured while sending the email")
        return False
